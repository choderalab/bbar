...
N_f = 1000; % number of forward realizations per experiment for fixed-number experiment
N_r = 1000; % number of reverse realizations per experiment for fixed-number experiment
...
for phase = 1:1 % FN, FP
  for replicate = 1:nreplicates	
    ...
    if (phase == 1)
        [df_mean, dfB_lower, dfB_upper] = BBAR_subsample(w_f, w_r, cis, P_f); % M-factor determined from fixed, known probability of forward/reverse work measurements
    else
...
% set(gca, 'YTick', [0 1]);
% 
% title('fixed probability experiment');
% 
...
filename = sprintf('../plots/bbar-subsample-sigma=%d-%d-%d.eps', sigma, N_f, N_r);
