...
clear;
N_f = 2; % number of forward realizations per experiment
N_r = 2; % number of reverse realizations per experiment

nreplicates = 200; % number of replications of the experiment to perform (the larger, the smaller the error bars in the plot)
...
for replicate = 1:nreplicates	
  ...
  [df_mean, dfB_lower, dfB_upper] = BBAR_test(w_f, w_r, cis);
...
filename = sprintf('../plots/confidence-%d-%d.eps', N_f, N_r);
